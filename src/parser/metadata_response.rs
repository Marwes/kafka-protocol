    ...
    I::Error: ParseError<I::Token, I::Range, I::Position>,
{
    (
        ...
        (
            ...
            (
                ...
                array(||be_i32().expected("offline_replicas").expected("offline_replicas"),),
    ...
    ).map(|(brokers,cluster_id,controller_id,topics,topics,cluster_authorized_operations,)| {MetadataResponse{
    brokers,cluster_id,controller_id,topics,topics,cluster_authorized_operations
    }})
}
...
impl<'i> crate::Encode for MetadataResponse<'i> where {
    fn encode_len(&self) -> usize {
        self.brokers.encode_len() + self.cluster_id.encode_len() + self.controller_id.encode_len() + self.topics.encode_len() + self.topics.encode_len() + self.cluster_authorized_operations.encode_len()}
...
impl<'i> crate::Encode for Brokers<'i> where {
    fn encode_len(&self) -> usize {
        self.node_id.encode_len() + self.host.encode_len() + self.port.encode_len() + self.rack.encode_len()}
    fn encode(&self, writer: &mut impl Buffer) {
...
impl crate::Encode for Partitions where {
    fn encode_len(&self) -> usize {
        self.error_code.encode_len() + self.partition_index.encode_len() + self.leader_id.encode_len() + self.leader_epoch.encode_len() + self.replica_nodes.encode_len() + self.isr_nodes.encode_len() + self.offline_replicas.encode_len()}
    fn encode(&self, writer: &mut impl Buffer) {
...
impl<'i> crate::Encode for Topics<'i> where {
    fn encode_len(&self) -> usize {
        self.error_code.encode_len() + self.name.encode_len() + self.is_internal.encode_len() + self.partitions.encode_len() + self.topic_authorized_operations.encode_len()}
    fn encode(&self, writer: &mut impl Buffer) {
        self.error_code.encode(writer);
